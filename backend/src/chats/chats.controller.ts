...
export class ChatsController {
  ...
  async createChat(
    ...
    const anonymousUserId = req.user?.type === 'anonymous' ? req.user?.id : null;

    const chat = await this.chatsService.createChat(userId, anonymousUserId, createChatDto);
    
    // Notify managers about new chat if it's newly created
    if (this.chatsGateway.server && chat.messages.length <= 2) { // New chat typically has 1-2 messages
      const chatListItem = await this.chatsService.formatChatListDto(chat as any, null);
  ...
  async getChatById(@Request() req, @Param('id') id: string): Promise<ChatDto> {
    ...
    return this.chatsService.getChatById(id, userId, anonymousUserId, isManager);
  }
  ...
  async sendMessage(
    ...
    const message = await this.chatsService.sendMessage(id, senderId, sendMessageDto);
  ...
  @ApiParam({ name: 'id', description: 'Chat ID' })
  @ApiResponse({ status: 200, type: ChatDto })
  async assignManager(@Request() req, @Param('id') id: string): Promise<ChatDto> {
  ...
  async createProductOffer(
    ...
    return this.chatsService.createProductOffer(id, req.user.id, createProductOfferDto);
  }
