...
export function UserCartTab({ userId }: UserCartTabProps) {
  ...
  const totalAmount = cart.totalPrice ? toNumber(cart.totalPrice) : cart.items.reduce((sum, item) => {
    const price = item.price || (item.product?.price ? toNumber(item.product.price) : 0);
    ...
    <div className="p-6">
      ...
      <div className="space-y-4">
        {cart.items.map((item) => {
          ...
          const price = item.price || (product?.price ? toNumber(product.price) : 0);
          const totalPrice = item.totalPrice || (price * item.quantity);
          ...
          if (isOffer && item.offer) {
            ...
            const productImage = item.product.productImages?.[0]?.url || item.product.images?.[0];
            imageUrl = productImage ? getImageUrl(productImage) : '/placeholder.svg';
          }
            ...
            <div key={item.id} className="flex gap-4 p-4 border rounded-lg">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {formatPrice(price)} × {item.quantity} = {formatPrice(totalPrice)}
                </p>
              </div>
            </div>
          );
