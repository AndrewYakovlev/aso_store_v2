...
### Система импорта товаров из Excel (2025-06-19)

- ✅ Создан backend модуль для импорта товаров:
  - ImportsModule с контроллером и сервисом
  - API endpoints для предварительного просмотра и импорта
  - Поддержка загрузки файлов через multipart/form-data
  - Ограничение размера файла до 10MB
- ✅ Реализован парсинг Excel файлов:
  - Автоматическое определение структуры файла
  - Обработка нестандартных заголовков колонок
  - Валидация данных и пропуск невалидных строк
  - Корректная обработка числовых значений
- ✅ Интеллектуальное сопоставление категорий и брендов:
  - Алгоритм расчета релевантности на основе совпадения слов
  - Настраиваемый порог уверенности (по умолчанию 70%)
  - Возможность указать категорию/бренд по умолчанию
  - Статистика сопоставлений в предварительном просмотре
- ✅ Гибкие настройки импорта:
  - Создание новых товаров / обновление существующих
  - Обновление только цен или только остатков
  - Пропуск ошибок или остановка при первой ошибке
  - Автоматическое или ручное сопоставление
- ✅ Frontend интерфейс в админ-панели:
  - Компонент ProductImportForm с пошаговым процессом
  - Предварительный просмотр с анализом данных
  - Настройка параметров импорта
  - Детальный отчет о результатах импорта
  - Визуализация статистики по категориям и брендам
- ✅ Обработка реального прайс-листа:
  - Успешный парсинг 4158 товаров из тестового файла
  - Автоматическое определение категорий для товаров
  - Распознавание брендов из названий товаров
