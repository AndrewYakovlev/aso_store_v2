...
### Управление марками автомобилей (2025-06-25)

#### Реализованные функции:

1. ✅ **Исправлено создание и редактирование марок автомобилей**
   - Добавлены методы create и update в vehicleBrandsApi на frontend
   - Методы принимают данные марки и токен авторизации
   - Использование POST для создания и PATCH для обновления

2. ✅ **Обновлен компонент VehicleBrandSheet**
   - Интеграция с API методами для сохранения данных
   - Добавлена проверка наличия токена авторизации
   - Использование slug как externalId при создании марки
   - Правильная обработка ошибок с информативными сообщениями

3. ✅ **Реализована загрузка логотипа марки**
   - Интеграция с существующим API uploadsApi для загрузки изображений
   - Загрузка файла происходит перед сохранением данных марки
   - URL загруженного файла сохраняется в поле logo
   - Обработка ошибок загрузки с отдельным сообщением

#### Технические детали:

- Backend API уже был готов и требовал только интеграции на frontend
- Использование токена авторизации для защищенных endpoints
- Загрузка изображений через универсальный endpoint /uploads/image
- Поддержка всех полей марки: name, nameCyrillic, slug, country, logo, popular, isActive, sortOrder
