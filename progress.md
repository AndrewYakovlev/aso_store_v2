...
### Chat System Implementation (2025-06-19)

- ✅ Backend модуль чатов:
  - Создан ChatsModule с полным функционалом
  - Поддержка анонимных и авторизованных пользователей
  - WebSocket Gateway для real-time сообщений
  - Система товарных предложений от менеджеров
  - API endpoints для всех операций с чатами
  - Автоматическое назначение менеджера на чат
  - Маркировка прочитанных сообщений
- ✅ Frontend компоненты чата:
  - ChatWidget для публичной части сайта
  - Интеграция с анонимными токенами
  - Real-time обновления через WebSocket
  - Страница управления чатами в админ-панели
  - ChatWindow - универсальный компонент чата
  - Отображение товарных предложений
  - Индикатор набора текста и статусы сообщений
- ✅ Интеграция WebSocket:
  - Socket.IO клиент и сервер
  - Автоматическое переподключение
  - События для новых сообщений и обновлений
  - Синхронизация между вкладками
  - Уведомления для менеджеров о новых чатах

### Chat Authentication Fix (2025-06-19)

- ✅ Исправлена авторизация в чате админ-панели:
  - Обновлен chatApi для передачи accessToken в методах sendMessage и markMessagesAsRead
  - ChatWindow теперь использует JWT токен менеджера вместо анонимного токена
  - Исправлены все API вызовы для использования правильных токенов
  - WebSocket подключение в админ-панели передает JWT токен менеджера

### Unified API Token System (2025-06-19)

- ✅ Создана единая система приоритета токенов:
  - UnifiedApiClient - централизованный клиент для API запросов
  - JWT токен авторизованного пользователя всегда в приоритете
  - Автоматическое переключение между JWT и анонимным токеном
  - AuthContext сохраняет токены в cookies для серверных компонентов
  - Middleware обновлен для поддержки обоих типов токенов
- ✅ Обновлены все API клиенты:
  - chatApi, favoritesApi, cartClientApi, ordersClientApi
  - Все клиенты используют UnifiedApiClient
  - Исправлена проблема с использованием анонимного токена после авторизации
  - Улучшена консистентность работы с токенами во всем приложении

### Chat Notification System (2025-06-19)

- ✅ Система уведомлений для чата:
  - NotificationContext для управления счетчиками непрочитанных
  - Звуковые уведомления при новых сообщениях
  - Визуальные индикаторы количества непрочитанных
  - Shake-анимация кнопки чата при новом сообщении
  - Автоматическая прокрутка к новым сообщениям
- ✅ Глобальные уведомления в админ-панели:
  - AdminNotificationListener - компонент для прослушивания всех чатов
  - Toast уведомления с помощью sonner
  - Отображение превью сообщения и имени отправителя
  - Кнопка быстрого перехода в чат
  - ActiveChatContext для предотвращения дублирования уведомлений
- ✅ Статусы доставки и прочтения сообщений:
  - Добавлены поля isDelivered/deliveredAt и isRead/readAt в БД
  - WebSocket события для синхронизации статусов
  - Визуальные индикаторы: ○ (отправляется), ✓ (доставлено), ✓✓ (прочитано)
  - API endpoints для отметки сообщений как прочитанных
  - Корректный подсчет непрочитанных после перезагрузки страницы
